...
public class InfluxRepository {
    ...
    private void batchWriteLoop() {
        List<Point> batch = new ArrayList<>(50); // 批次大小 50

        while (running) {
            try {
                // 从队列取数据，如果队列空了，等待 1 秒（用于触发定时写入）
                Point point = pointQueue.poll((long) 0.0001, TimeUnit.SECONDS);
                if (point != null) {
                    batch.add(point);
                    // 如果攒够了 50 条，或者队列里还有很多，立即写入
                    if (batch.size() >= 50 || pointQueue.size() > 100) {
                        flushBatch(batch);
                    }
                } else {
                    // 超时未取到数据，如果批次里有残留数据，也写入（防止数据积压在内存）
                    if (!batch.isEmpty()) {
                        flushBatch(batch);
                    }
                }
    ...
    public void saveTelemetryAsync(String deviceId, Map<String, Object> fields) {
        if (pointQueue.remainingCapacity() == 0) {
            System.err.println("⚠️ 写入队列已满，丢弃数据！");
